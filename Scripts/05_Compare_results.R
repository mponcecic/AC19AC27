...
# Correlation 
cor_all <- cor.test(x = gdt$log2FC_AC27[-which(is.na(gdt$Congruence))], gdt$log2FC_AC19AC27[-which(is.na(gdt$Congruence))])

ggplot(gdt, aes(x = log2FC_AC27, y = log2FC_AC19AC27, color = factor(Congruence), shape = Shared, label = Labeled))+
  geom_point(size = 2.5)+
...
data <- gdt %>% select(Name, Shared, Congruence, log2FC_AC27, padj_AC27, log2FC_AC19AC27, padj_AC19AC27)
...
#####################################################################################################
# CHECK THE LIST OF GENES
#####################################################################################################

raw_data[which(raw_data$Symbol %in% c("ATF4", "CHAC1", "SESN2", "ALDH1L2", "CBS", "PHGDH")), c("Name", "Symbol", "Ensembl", "Biotype", "DEG", "log2FC", "padj")]

# Save
write.table(raw_data[which(raw_data$Symbol %in% c("ATF4", "CHAC1", "SESN2", "ALDH1L2", "CBS", "PHGDH")),], paste("Listofgenes_", comparison, "_", project, "_", analysis_ID, ".txt", sep = ""), quote = FALSE, row.names = FALSE, sep = "\t")
