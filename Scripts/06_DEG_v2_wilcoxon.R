...
for (i in 1:length(contrast)){
  ...
  data <- data %>% select(GENEID, Symbol, EnsemblID, DEG, Direction, logFC, pvalue, padj, everything())
  ...
  sel <- cbind(sel, m)
  sel <- sel %>% select(GENEID, Symbol, EnsemblID, DEG, Direction, logFC, pvalue, padj, everything())
