...
# script.
# 
# In this script, Wilcoxon method is perform based on the gene count matrix resulting 
...
for (i in 1:length(contrast)){
  ...
  result2 <- result2 %>% select(Comparison, Name, Symbol, Ensembl, DEG, Direction, logFC, padj, pvalue, everything())
...
colnames(final_data) <- c("Name", "Symbol", "Ensembl", "DEG", "Direction", "logFC", "padj", "pvalue", col_raw)
write.table(final_data, file = paste(dir_infiles, "/", analysis, "_", project, ";All_", threshold, ".txt", sep = ""), sep = "", eol = "\t", row.names = FALSE, col.names = TRUE)
